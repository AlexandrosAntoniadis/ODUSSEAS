...
def ML(regression, reference):
    ...
    for i in np.arange(len(filepaths)):
        ...
        newlines = np.loadtxt(
            "./"
            + directory_name
            + filepaths[i].replace(".fits", "").replace("spectra/" + "newstars/", "")
            + "centrallines.dat",
            dtype=str,
        )
        ...
        if resolution[i] == "115000" and reference == "interferometry":
            wide_error_FeH = ((np.std(FeH_list)) ** 2 + (0.11) ** 2) ** (1 / 2)
        ...
        res_file.write(
            str(newlabels.iat[0, 0])
            + " "
            + str(round(np.mean(FeH_list), 3))
            + " "
            ...
            + " "
            + str(int(np.mean(Teff_list)))
            + " "
            + str(int(np.std(Teff_list)))
            + " "
            + str(int(np.mean(MAE_Teff_list)))
            + " "
            + str(int(wide_error_Teff))
            + " "
            + str(round(np.mean(R2_list), 3))
            + " "
        ...
        plt.savefig(
            "./" + MLplots_folder + "/" + starname + "_FeH_test_comparison.pdf",
            bbox_inches="tight",
        )
        plt.close()
        ...
        ax.spines["top"].set_visible(True)
        plt.savefig(
            "./" + MLplots_folder + "/" + starname + "_Diff_FeH_test_comparison.pdf",
            bbox_inches="tight",
        )
        plt.close()
        ...
        plt.savefig(
            "./" + MLplots_folder + "/" + starname + "_Teff_test_comparison.pdf",
            bbox_inches="tight",
        )
        plt.close()
        ...
        ax.plot(
            test_predvalues.values[:, 2],
            test_predvalues.values[:, 2] - test_givenvalues.values[:, 2],
            "ko",
        )
        ax.plot(clip_box=True, clip_on=True)
        plt.savefig(
            "./" + MLplots_folder + "/" + starname + "_Diff_Teff_test_comparison.pdf",
            bbox_inches="tight",
        )
        plt.close()
    res_file.close()
