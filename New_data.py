...
def EWmeasurements(do_rv_cor):
    ...
    for i in np.arange(len(filepaths)):
        os.system(
            ...
            + filepaths[i]
            + " "
            + filepaths[i].replace(".fits", "final")
            + ""
            + ".fits"
        )
    ...
    for i in np.arange(size_of_filepaths):
        ...
        starwavelength, _ = read_data(filepaths[i])
        starwavelength_min, starwavelength_max = np.min(starwavelength), np.max(
            starwavelength
        )
