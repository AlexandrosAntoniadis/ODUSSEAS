...
wavelength_range = np.loadtxt(inst+'_lines.rdb', skiprows=2)
dw = 0.4
plot = False

directory_name = inst+'_EWmyresults'
...
np.savetxt(inst+'_centrallines.dat', wcentral)
lines = np.loadtxt(inst+'_centrallines.dat')
...
np.savetxt(inst+"_EW.dat", table, header = headers, delimiter=",")   
    
     # transpose
     
table = np.loadtxt(inst+'_EW.dat', dtype=str, delimiter=',', comments="?")

table_T = np.transpose(table)
table_T[0,0] = table_T[0,0].replace('# ','')
print(table_T[0])
np.savetxt(inst+'_newstars.csv', table_T, delimiter=',', fmt='%s')
